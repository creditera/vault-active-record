...
describe Vault::ActiveRecord do
  before(:all) do
    Vault::ActiveRecord.sys.mount("transit", :transit)
  end

  context "with default options" do
    before(:all) do
      Vault::ActiveRecord.logical.write("transit/keys/dummy_people_ssn")
    end
    ...
    it "does not try to encrypt unchanged attributes" do
      ...
      expect(Vault::ActiveRecord).to_not receive(:encrypt)
  ...
  context "with custom options" do
    before(:all) do
      Vault::ActiveRecord.sys.mount("credit-secrets", :transit)
      Vault::ActiveRecord.logical.write("credit-secrets/keys/people_credit_cards")
    end
  ...
  context "with non-ASCII characters" do
    before(:all) do
      Vault::ActiveRecord.sys.mount("non-ascii", :transit)
      Vault::ActiveRecord.logical.write("non-ascii/keys/people_non_ascii")
    end
  ...
  context "with the :json serializer"  do
    before(:all) do
      Vault::ActiveRecord.logical.write("transit/keys/dummy_people_details")
    end
    ...
    it "encodes and decodes attributes" do
      ...
      raw = Vault::ActiveRecord.decrypt("transit", "dummy_people_details", person.details_encrypted)
  ...
  context "with a custom serializer" do
    before(:all) do
      Vault::ActiveRecord.logical.write("transit/keys/dummy_people_business_card")
    end
    ...
    it "encodes and decodes attributes" do
      ...
      raw = Vault::ActiveRecord.decrypt("transit", "dummy_people_business_card", person.business_card_encrypted)
  ...
  context "with custom encode/decode proc" do
    before(:all) do
      Vault::ActiveRecord.logical.write("transit/keys/dummy_people_favorite_color")
    end
    ...
    it "encodes and decodes attributes" do
      ...
      raw = Vault::ActiveRecord.decrypt("transit", "dummy_people_favorite_color", person.favorite_color_encrypted)
